...
### 編集操作の整理(3ジェスチャーの分離)
写真タップで調整画面へ移行する方式は、移動/拡大との誤爆が起きやすいため廃止。以下に整理:
- [x] **通常モード**: ドラッグ=写真を移動 / ピンチ=拡大縮小 / **長押し(0.5秒)=入れ替え**。タップは無反応
- [x] **写真の入れ替え** — 長押しで写真が浮き(振動フィードバック)、ドラッグ&ドロップで別セルと位置を交換。
  変形状態は写真 ID に紐付くため各写真は自分のズーム・位置を保ったまま移動。Undo 対応(並び順を ID 列でスナップショット)
- [x] **調整モード** — 下部の「写真を調整」ボタンで明示的に切替。この間は入れ替えを無効化して誤爆を防ぐ
...
- [x] **水平・回転補正** — 写真ごとの傾き調整(Pro)
  - 「写真を調整」ボタンで調整モードへ。選択セルに三分割グリッド+白枠を表示(複数枚は別写真タップで対象切替)
  - 水平スライダー(±15°)・90°回転・リセット。傾けても背景が見えないよう自動で最小限拡大
  - 回転は `CellTransform.rotationDegrees` に保存(90°単位+微調整の合成値)。
    プレビュー(rotationEffect)と書き出し(CGContext回転)は同じ `CellGeometry` の矩形を共有
  - 未購入時はパネルがロック表示になりペイウォールへ
