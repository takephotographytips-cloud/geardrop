...
import { ExternalLink, ArrowLeft, Calendar, ChevronRight, Clock } from "lucide-react";
...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
    ...
    const { data: article } = await supabase
        .from('articles')
        .select('title, summary_text, thumbnail_url, published_at')
        .eq('id', id)
        .single();
...
export default async function ArticlePage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const structuredData = {
        ...
        "author": [{
            ...
            "url": new URL(article.source_url).origin
        }],
        "description": article.summary_text
    };
                ...
                <div className="grid grid-cols-1 md:grid-cols-[1fr_400px] gap-8 bg-card rounded-2xl border border-border/50 overflow-hidden shadow-lg">
                    ...
                    <div className="p-8 flex flex-col">
                        <div className="flex gap-2 mb-4">
                            <span className="px-3 py-1 text-sm font-semibold bg-primary/10 text-primary border border-primary/20 rounded-md">
                        ...
                        <h1 className="text-2xl md:text-3xl lg:text-4xl font-extrabold tracking-tight mb-4 leading-snug">
                            {article.title}
                        </h1>

                        {/* Action Bar */}
                        <div className="flex flex-wrap items-center gap-4 mb-8 pt-4 border-t border-border/50">
                            <BookmarkButton
